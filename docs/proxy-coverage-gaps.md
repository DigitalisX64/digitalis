...
| libnativehelper | 31 | ~17 (JniConstants_*/JniInvocation*) | 13 (12 covered, 1 hard) |
...
`DoBadTrampoline` symbol is now covered (libnativehelper ×12, libbinder_ndk ×2,
...
- **libnativehelper — COVERED (12):** `jniThrowException`, `jniThrowNullPointerException`, `jniThrowRuntimeException`, `jniThrowIOException`, `jniThrowErrnoException`, `jniLogException`, `jniCreateString`, `jniGetNioBufferFields`, `jniGetNioBufferPointer`, `jniGetNioBufferBaseArray`, `jniGetNioBufferBaseArrayOffset`, `jniRegisterNativeMethods` — the public `<nativehelper/JNIHelp.h>` C API. Each takes a `JNIEnv*` first argument, which is NOT a plain pointer pass-through (the guest `JNIEnv` holds guest-callable function pointers); the custom trampolines in `digitalis_extra_proxy/digitalis_extra_libnativehelper_trampolines.cc` translate it with `ToHostJNIEnv` and forward the remaining flat args (`jclass`/`jobject`/`const char*`/`jint*` pass through verbatim under LP64). They appeared incompatible only because the API analysis didn't resolve the JNI types. `jniRegisterNativeMethods` likewise forwards its `JNINativeMethod` array verbatim — the guest native function pointers it carries are left unwrapped exactly as the native bridge's own `JNIEnv::RegisterNatives` trampoline leaves them; the ART native-bridge layer routes their later invocation through the translator. The host function is reached via the dlsym'd `callee`, so libberberis_arm64.so gains no libnativehelper link dependency. Validated end-to-end by the hello-jni `[LIBNH-PROXY:PASS ... reg_natives=1]` probe.
...
- libnativehelper: **done** — 12 public `<JNIHelp.h>` C-API symbols covered in
  `digitalis_extra_libnativehelper_trampolines.cc` (JNIEnv translation via
  `ToHostJNIEnv`, including `jniRegisterNativeMethods`); validated by the
  hello-jni `[LIBNH-PROXY:PASS ... reg_natives=1]` probe. Only
  `jniThrowExceptionFmt` (true varargs) remains.

...
| Scalar FP | FP16 `FMADD`/`FMSUB`/`FNMADD`/`FNMSUB` (double-rounding unsafe via F16C round-trip — must stay a bail); FP16 scalar↔int converts | The F16C round-trip is correctly rounded for +,−,×,÷,√ (FP32's mantissa ≥ 2·11+2) but NOT for fused multiply-add — lowering FP16 FMA that way would double-round, so it must stay a bail. |
| NEON (residual) | all `.2D`/64-bit-lane integer forms (§3a.1, hardware-conditional); the FP16 vector residue — FMLA/FMLS (double-rounding unsafe), FMULX/FRECPS/FRSQRTS, the FP16 pairwise family, FP16 FCADD/FCMLA, and the FP16 vector converts/estimates; `FCVTXN` (round-to-odd needs MXCSR access the heavy machine IR does not expose; correct via lite); scalar saturating shifts `SQSHL`/`UQSHL`/`SQRSHL`/`UQRSHL` | Known tier-consistent quirk (not a bail): lite and heavy FP16 FMAX/FMAXNM return −0h on the ±0 tie where ARM specifies +0h (the FP32/FP64 paths are correct); a coordinated lite+heavy fix is a follow-up. |

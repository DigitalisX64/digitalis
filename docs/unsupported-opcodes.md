...
> **Status re-verified 2026-07-19** after a six-slice heavy-tier coverage batch: FCSEL re-landed (the historical region miscompile was the stale-forwarded-vreg class, fixed since — the new lowering blends in a private temp and is correct either way); the scalar FP16 surface now lowers via the F16C round-trip (FP16 FMA stays bailing — double-rounding unsafe); vector FABS/FNEG/FSQRT (FP32+FP64), FACGE/FACGT, FCVTL/FCVTN (FP32↔FP64), FRECPE/FRSQRTE/SQABS/SQNEG; SMMLA/UMMLA/USMMLA, BFDOT/BFMMLA/BFMLALB/T (vector+indexed), indexed FCMLA (FP32); byte/halfword LSE atomics plus CASP-64 and LDXP/STXP (32- and 64-bit pairs via CMPXCHG16B — the heavy tier's first four-fixed-register op); byte-lane MUL/MLA/MLS and byte SSRA/USRA/URSHR; scalar three-same integer D-forms, scalar pairwise, and scalar by-element FMUL/FMLA/FMLS/FMULX. §3a below reflects the post-batch state.
>
> **Prior status (2026-07-14)** against the current decoder, lite translator (`.inc` split), and heavy optimizer (`frontend.cc`). §2 (SVE/SME/FP8 absent) is unchanged. §3 changed: **AES (`AESE/AESD/AESMC/AESIMC`) is now JIT-lowered via AES-NI** (lite + heavy) — only SHA/SM3/SM4 and IEEE-CRC32 remain interpreter-only crypto; scalar by-element `SQDMULH`/`SQRDMULH`/`SQDMULL` (previously `Undefined()`→`SIGILL`) are now implemented. §3a (heavy-tier coverage) was expanded again — the LSE bitwise + min/max atomics + `CASP` 32-bit pair, `CRC32C`, `SDOT`/`UDOT`, and FP32 `FCADD`/`FCMLA` now lower in the second gear (the heavy frontend gained its first internal `CMPXCHG` back-edge loop). §3a.1 (hardware-conditional `.2D`) is unchanged.
...
| Scalar FP | FP16 `FMADD`/`FMSUB`/`FNMADD`/`FNMSUB` (double-rounding unsafe via F16C round-trip — must stay a bail); FP16 scalar↔int converts; `BFCVT`; FP64 `FCVTAS`/`FCVTAU`; the `rmode=01` top-half `FMOV V.D[1]` | `FCSEL` (S/D) is re-landed with a private-temp blend — the historical region miscompile was the stale-forwarded-guest-context-vreg bug, since fixed in the optimizer, and the new lowering never mutates the cached source reads so it is correct either way. The scalar FP16 surface (arith, min/max, `FSQRT`, `FRINT*`, compares, `FCVT` to/from half, `FMOV` incl. immediate) now lowers via the F16C round-trip (F16C-gated; correctly rounded for +,−,×,÷,√ since FP32's 24-bit mantissa ≥ 2·11+2). |
| NEON (residual) | all `.2D`/64-bit-lane integer forms (§3a.1, hardware-conditional); FP16 *vector* forms (`.4h`/`.8h` — recipe documented, deferred); `URECPE`/`URSQRTE` and `FCVTXN`; byte-lane `SLI`/`SRI`/`SRSHR`/`SRSRA`/`URSRA` (bail in lite too); scalar three-same beyond the integer D-forms (`SQADD`/`UQADD` scalar, `SSHL`/`USHL` scalar, `FABD`/`FMULX`/`FRECPS`/`FRSQRTS` scalar, `SQRDMLAH`/`SQRDMLSH` scalar); scalar D-width saturating narrows | Newly lowered this batch: vector `FABS`/`FNEG`/`FSQRT` (FP32+FP64), `FACGE`/`FACGT`, `FCVTL`/`FCVTN` (FP32↔FP64), `FRECPE`/`FRSQRTE`/`SQABS`/`SQNEG`; byte-lane `MUL`/`MLA`/`MLS` and byte `SSRA`/`USRA`/`URSHR`; scalar three-same integer D-forms (`ADD`/`SUB`/`CMEQ`/`CMGT`/`CMGE`/`CMHI`/`CMHS`/`CMTST`); scalar pairwise (`ADDP`-D, `FADDP`/`FMAXP`/`FMINP`/`FMAXNMP`/`FMINNMP`); scalar by-element `FMUL`/`FMLA`/`FMLS`/`FMULX`. |

...
`sample/hellodigitalis/` holds 158 ARM64-only sample app modules — the integration test suite. They span ndk-samples ports, proxy-lib smoke tests (including NDK proxy-surface probes: AHardwareBuffer, AImageDecoder, AMediaDataSource/Muxer, ADPF, ASharedMemory, system fonts, OpenMAX AL, OpenSL ES), ARM-extension probes, UI engines (Qt 6, React Native, Lynx), and third-party native libraries; `ls` the directory for the current set. All 158 are exercised by `test-samples.sh` (`hello-realm` is a standalone pinned-toolchain Gradle project; the suite auto-builds it via its `build-apk.sh` and verifies it by launch).
...
| `sample/hellodigitalis/` | The 158 sample app modules |
...
- **Discovery covers the root plus the staging areas, de-duplicated by package.** The gate discovers root `*.apk` files, root split-app subdirectories, **and** the `top-apps/`/`top-games/` staging areas of `digitalis/scripts/fetch-prebuilt-apks.py` (scanned one level deep — their own `*.apk` files and their split-app subdirs), so every verified app is a regression target. Targets are de-duplicated by package name, first occurrence winning, so an app promoted to a root entry is tested from there and its staging copy is skipped rather than launched twice. A directory whose APKs span more than one package (`benchmark-apps/`, which holds an arm64 and an x86_64 build of both 3DMark and Geekbench) is not one app's split group, so its APKs are enrolled **individually** rather than skipped; per-package de-duplication then keeps the arm64 build, because `-arm64-v8a` sorts before `-x86_64`, and leaves the native build out as the benchmark denominator it exists to be. Keep discovery generic — no hard-coded app names.
- **Every target must actually be translated.** The device ABI list is `x86_64,arm64-v8a,x86`, so a multi-ABI "universal" APK carrying an x86/x86_64 slice would install as the HOST abi and run natively — exercising nothing while still reporting green. Installs are pinned with `--abi arm64-v8a` (and the `install-multiple` equivalent), which selects the arm64 slice with no repackaging and no re-signing; a `primaryCpuAbi` check afterwards reports SKIP for anything that still landed on a host abi. Never strip a slice out of an APK to "make it arm64-only": repacking forces a re-sign that breaks signature and anti-tamper checks.
- **The gate guards against emulator exhaustion, and aborts on a dead device.** A full sweep installs and launches 130+ apps into one emulator; accumulated packages and background services eventually make HEALTHY apps fail (blank frames, start timeouts, SIGSEGVs that vanish on reboot) and those read as translator regressions. The dominant cost is **leftover running apps** (one sweep had a single title holding 889 MB, ~1.5 GB across leftovers), so `MemAvailable` is checked before each target and the response is reclaim-first: force-stop the third-party processes that are actually running, which returned 1.6 GB in seconds when measured. Only if that is not enough does it reboot, and not more often than a cooldown (`PREBUILTS_MEM_FLOOR_KB` default 700 MB, `PREBUILTS_REBOOT_COOLDOWN` default 15 targets) — a low floor on a busy device would otherwise thrash into one reboot per target. If adb has no live device the run aborts non-zero instead of recording a wall of "(install failed)" — a dead emulator has no result to report.
- **A sweep-time FAIL is a lead, not a finding — reproduce it standalone on a rested device before doing anything else (binding).** The exhaustion guard lowers the flake rate; it does not eliminate it, and an exhausted emulator fails *healthy* apps in exactly the shapes a translator bug has: SIGSEGV in guest code, a blank first frame, a start timeout, a process that vanishes. So before tracing, bisecting, A/B-ing a translator change, or writing any root cause down: reclaim or reboot, install that one target alone, launch it, and watch it (`pidof` over ~60 s, `topResumedActivity`, the crash-marker grep, and a screenshot). Only a standalone reproduction earns an investigation. **A tier ladder is not an isolation technique here.** `berberis.mode interpret-only` changes timing and memory pressure, so it "fixes" any exhaustion flake, and reading that as "the bug is in the lite translator" is a false positive that has cost whole cycles — four Unity titles once localised that way all passed at 56/60 on a freshly booted emulator with zero translator changes. When a conclusion turns out to have rested on an unreproduced flake, retract it in as many words and correct wherever it was recorded; do not quietly move on.
- **An incomplete or mis-packaged APK is a defect to repair, not a label to file it under (binding).** Triage that stops at "bad prebuilt" leaves a standing red line in the gate, and a gate with permanent failures stops being read at all — which is how a real regression gets to hide. When the artifact is the problem — `INSTALL_FAILED_MISSING_SPLIT`, stripped `META-INF/services` registrations (a `ServiceLoader` lookup then throws at app init), missing drawable resources, a repack that broke the signature, a universal build whose host slice made the run native — **fix the artifact**: re-fetch a complete copy (the vendor-signed universal APK, the full `.apkm`/split set — see `digitalis/scripts/fetch-prebuilt-apks.py`), replace the file under `sample/prebuilts/`, re-run that one target, and record the result you then observe. Only when a complete copy is genuinely unobtainable does it become a classification, and then state what was tried and either drop the target from the staging area or record it with its reason in the site's `blocked` list. Never end a cycle having merely *named* a broken prebuilt.
...
- **Report what you observed, name the gate that produced it, and read the whole error before naming a cause (binding).** The prebuilt gate proves a *launch*: the process survived and printed no crash marker. That is not the same as the app working, so never promote a gate PASS to "verified" in the release notes or on the website without looking at the app — a screenshot showing its own UI, not the launcher behind it. In the other direction, never publish a count, a PASS/FAIL, or a root cause that a rested-device run did not actually produce, and attach the device state it was measured at (`MemAvailable`, reclaim and reboot counts); a number with no device state behind it is not reproducible. When an app dies, read the **entire** exception chain to its last `Caused by:` before attributing blame — the first frames are routinely unrelated warnings, and calling an OpenTelemetry `ServiceLoader` failure "a GMS dependency" off the top of a trace was exactly that mistake. Uncertainty is reportable ("did not reproduce; cause unknown"); a confident wrong cause is not.

...
- **Commit verified clean code aggressively — even partial fixes.** Each dispatch cycle that produces code that builds clean (host tests + `m libberberis_arm64`), passes its target test (sample suite still PASS, the new sample's probe reports OK, or the host gtest landed), and doesn't regress the existing PASS count should be committed before the cycle ends. Don't accumulate uncommitted work across cycles waiting for a "complete" fix. Long-running uncommitted work is a hazard: stray `git stash` / `git reset` can silently delete hours of code, bisect requires per-feature history, and progress is invisible if it doesn't land. "Partial" is fine — `interpreter: implement FCADD vector (FP32 only; FP16 follow-up)` is a good commit; the FP16 case is the next cycle's commit. One commit per logical change across the three sub-repos (`frameworks/libs/binary_translation/`, `sample/hellodigitalis/`, `digitalis/`).

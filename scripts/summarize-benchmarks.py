...
# Not a translation mode: the same workload built for the host ABI and run
# without the translator. It is the denominator that answers "is translated code
# fast enough to use", which a speedup over the interpreter cannot.
NATIVE = "native"
...
def write_markdown(path, source, records, grouped, modes_seen):
    ...
    jit_modes = [m for m in modes_seen if m not in (BASELINE, NATIVE)]
    has_native = NATIVE in modes_seen
    # Explicit column order; sorted() would file native between the tiers.
    columns = [BASELINE] + jit_modes + ([NATIVE] if has_native else [])
    ...
    for (module, case), by_mode in grouped.items():
        ...
        native_median, native_noisy = stats_for(by_mode.get(NATIVE))
        ...
        if has_native:
            if native_median:
                cells.append(("⚠ " if native_noisy else "") + fmt(native_median))
            else:
                cells.append("—")

        vs_native = "—"
        if has_native and native_median and best_mode and not native_noisy:
            ratio = reliable[best_mode] / native_median
            vs_native = (f"{ratio:.1f}× slower" if ratio >= 1.0
                         else f"{1 / ratio:.1f}× faster")
        elif has_native and native_median and best_mode:
            vs_native = "~ (native noisy)"

        label = f"**{module.removeprefix('hello-')}** {case}"
        rows.append((speed, label, cells, best, vs_native))

    rows.sort(key=lambda r: (-r[0], r[1]))

    header = ["module / workload"] + [BASELINE_LABEL.get(m, m) for m in columns] + ["best vs interp"]
    if has_native:
        header.append("best vs native")
    lines = [
        "# Benchmark Results",
        "",
        f"Run `{stamp}` on `{product}` — {len(records)} records, "
        f"{len(grouped)} workloads, modes: {', '.join(BASELINE_LABEL.get(m, m) for m in columns)}.",
        "",
    ...
    for _, label, cells, best, vs_native in rows:
        tail = [best] + ([vs_native] if has_native else [])
        lines.append("| " + " | ".join([label] + cells + tail) + " |")
    lines.append("")

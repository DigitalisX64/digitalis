...
- **Check which process died before tracing anything.** A `FATAL EXCEPTION` with `is for EM_AARCH64 (183) instead of EM_X86_64 (62)` from a pid that is not one of the app's zygote-forked processes is an app-started `app_process64 / <Class>` helper (a keep-alive daemon, for example). ART keeps the native bridge only in zygote-forked processes, so that helper runs with no Digitalis at all. It is a documented limitation (how-it-works.md §5), not a translator bug, and there is nothing to trace. Confirm with `ps -A -o PID,PPID,NAME,ARGS` (parent is `sh` or the app, not `zygote64`) and `/proc/<pid>/environ` (`CLASSPATH=…/base.apk`). The app's own processes keep running; screenshot the app before classifying.
